...
async function loadAdminTab(t){
    ...
    if(t==='investments'){
        ...
        tb.innerHTML=`<div class="glass-card" style="padding:20px;"><table><tr><th>User</th><th>Plan</th><th>Amount</th><th>Profit</th><th>Days Left</th><th>Status</th></tr>${all.map(i=>`<tr><td>${i.ue}</td><td>${i.plan}</td><td>$${i.amount}</td><td>+$${i.dailyProfit}/d</td><td>${i.daysRemaining}d</td><td>${i.status}</td></tr>`).join('')}</table></div>`;
    }
    if(t==='referrals'){
        const all=await fsGet('referrals',null,500);
        const tb=document.getElementById('admin-referrals-tab');
        tb.innerHTML=`<div class="glass-card" style="padding:20px;"><table><tr><th>Referrer ID</th><th>Referred ID</th><th>Commission</th></tr>${all.map(r=>`<tr><td>${r.referrerId}</td><td>${r.referredUserId}</td><td>$${r.commission||0}</td></tr>`).join('')}</table></div>`;
    }
    if(t==='chats'){
        const tb=document.getElementById('admin-chats-tab');
        tb.innerHTML=`<div class="glass-card" style="padding:20px;"><h3>💬 Live Chats</h3><p style="color:#94a3b8;">Open chat bubble to view and reply to user messages.</p></div>`;
    }
    if(t==='settings'){
        const tb=document.getElementById('admin-settings-tab');
        const ps=await getPaymentSettings();
        tb.innerHTML=`<div class="glass-card" style="padding:20px;"><h3>💱 Exchange Rate</h3><input id="er" value="${ps.er||278.50}" style="padding:10px;background:rgba(0,0,0,0.5);border:1px solid #667eea;border-radius:8px;color:#fff;"><button onclick="saveER()" class="btn-primary btn-sm">Save</button></div>`;
    }
    if(t==='paymethods'){
    ...
    if(t==='plans'){
        ...
        tb.innerHTML=`<div class="glass-card" style="padding:20px;"><h3>📊 Investment Plans</h3>${investmentPlans.map((p,i)=>`<p>${p.icon} ${p.name} - $${p.amount} | ${p.pct}% | ${p.days}d | <button onclick="deletePlan(${i})">🗑️</button></p>`).join('')}<button onclick="createNewPlan()" class="btn-success btn-sm">➕ New Plan</button></div>`;
    }
}
...
async function rejectWithdraw(id){
    const w=(await fsGet('withdrawals')).find(x=>x.id===id);
    if(w){await fsUpdate('withdrawals',id,{status:'rejected'});const u=await db.collection('users').doc(w.uid).get();if(u.exists)await fsUpdate('users',w.uid,{balance:(parseFloat(u.data().balance)||0)+(parseFloat(w.amount)||0)});loadAdminTab('withdrawals');loadAdminStats();}
}
...
async function createNewPlan(){
    const n=prompt('Plan name:'),a=parseFloat(prompt('Amount ($):')),p=parseFloat(prompt('Daily %:')),d=parseInt(prompt('Days:'));
    if(n&&a&&p&&d){investmentPlans.push({name:n,amount:a,pct:p,days:d,icon:'💰'});await db.collection('settings').doc('investment_plans').set({plans:investmentPlans});loadAdminTab('plans');refreshInvestPage();}
}
async function saveER(){const er=document.getElementById('er').value;const s=await getPaymentSettings(true);s.er=parseFloat(er);await db.collection('payment_settings').doc('payment_settings_doc').update({er:s.er});showAlert('Exchange Rate Saved!','success');}
